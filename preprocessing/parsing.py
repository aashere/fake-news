...
def parse_query_data():
    fake_data = None
    real_data = None

    with open('data/raw/twitter_data/fake_twitter_raw.json', 'r') as f:
        fake_data = json.loads(f.read())
    with open('data/raw/twitter_data/real_twitter_raw.json', 'r') as f:
        real_data = json.loads(f.read())

    fake_df = query_parse_helper(fake_data)
    real_df = query_parse_helper(real_data)

    fake_df.to_csv('data/processed/twitter_parsed/fake_twitter_parsed.csv', index=False)
    real_df.to_csv('data/processed/twitter_parsed/real_twitter_parsed.csv', index=False)
    
            
def parse_reply_data():
    pass